...
// Health Check エンドポイント（Render用）
app.get('/health', (_req, res) => res.status(200).send('OK'));

// ① ルート（/）アクセス時に public/index.html を返す
app.get('/', (_req, res) => {
  res.sendFile(path.join(__dirname, 'public', 'index.html'));
});

// ② public フォルダの静的ファイルを配信（CSS/JS/画像など）
app.use(express.static(path.join(__dirname, 'public')));

// JSON 処理 + rawBody パース (LINE署名検証用)
  ...
  async (req, res) => {
    ...
    for (const event of events) {
      ...
      if (text.startsWith('タスク追加 ')) {
        ...
        // 返信 & プッシュ
        await lineClient.replyMessage(event.replyToken, { type: 'text', text: 'タスクを追加しました！全タスクを送信します…' });
        for (const t of allTasks) {
          await lineClient.pushMessage(userId, { type: 'text', text: `📌 ${t.task}（${t.date||'未定'} ${t.time||''}）` });
        }
      }
      // 進捗確認
      else if (text === '進捗確認') {
...
app.post('/add-task', async (req, res) => {
  ...
  await supabase.from('user_settings').upsert({ user_id: userId, notify: true });
  const { error: insErr } = await supabase.from('todos').insert({ user_id: userId, task, status: '未完了', date: date||null, time: time||null });
  if (insErr) return res.status(500).json({ success: false, error: '登録に失敗しました' });
  try {
    const { data: settings } = await supabase.from('user_settings').select('notify').eq('user_id', userId).single();
    if (settings?.notify) {
      await lineClient.pushMessage(userId, { type: 'text', text: `🆕 タスクが追加されました！\n${task}\n期限：${deadline||'未定'}` });
    }
  } catch(e) {
    console.warn('[add-task push error]:', e.message);
  }
...
cron.schedule('0 9 * * *', async () => {
  ...
  try {
    const { data: tasks, error } = await supabase.from('todos').select('*').eq('status','未完了').order('date',{ ascending:true });
    ...
    for (const [uid,list] of Object.entries(byUser)) {
      ...
      for (const t of list) {
        await lineClient.pushMessage(uid,{ type:'text', text:`🔔 今日のタスク\n${t.task}\n期限：${t.date||'未定'} ${t.time||''}` });
      }
    }
